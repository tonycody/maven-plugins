...
/**
 * This is the result returned from the verify operation. It contains a status and a error message
 * (if applicable)
 *
 * @author bjorn
 * @since 2012-08-11
...
public final class XmlOrderedResult {
    ...
    public static XmlOrderedResult nameDiffers(String originalElementName, String newElementName) {
        return new XmlOrderedResult(
                false,
                String.format(
                        "The xml element <%s> should be placed before <%s>", newElementName, originalElementName));
    }
    ...
    public static XmlOrderedResult textContentDiffers(String name, String originalElementText, String newElementText) {
        return new XmlOrderedResult(
                false,
                String.format(
                        "The xml element <%s>%s</%s> should be placed before <%s>%s</%s>",
    ...
    public static XmlOrderedResult lineDiffers(int lineNumber, String sortedXmlLine) {
        return new XmlOrderedResult(
                false, String.format("The line %d is not considered sorted, should be %s", lineNumber, sortedXmlLine));
    }
