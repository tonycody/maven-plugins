...
class SortOrderTest {
    ...
    final void testSortXmlReal1() throws Exception {
        SortPomImplUtil.create().noKeepBlankLines().testFiles("/Real1_input.xml", "/Real1_expected.xml");
    }
    ...
    final void testSortWithIndent() throws Exception {
        SortPomImplUtil.create().nrOfIndentSpace(4).testFiles("/Simple_input.xml", "/Simple_expected_indent.xml");
    }
    ...
    final void corruptFileShouldThrowException() {
        ...
        assertThat(
                "Unexpected message",
                thrown.getMessage(),
                allOf(endsWith("content: <project><artifactId>sortpom</artifactId>"), startsWith("Could not sort ")));
    }
}
